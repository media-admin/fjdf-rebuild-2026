...
/**
 * Video Testimonial Block — wiederverwendbar auf Homepage + Was wir tun
 * @param int $post_id Post-ID von der die ACF-Felder geladen werden
 */
function fjdf_video_testimonial( int $post_id = 6 ): void {
	$test_label    = fjdf_field( 'fjdf_testimonial_label',    $post_id, 'ZEUGNIS' );
	$test_headline = fjdf_field( 'fjdf_testimonial_headline', $post_id, __( 'Erfahren Sie mehr über die Erlebnisse unserer Begünstigten', 'fjdf' ) );
	$testimonials  = fjdf_field( 'fjdf_testimonials',         $post_id, [] );
	$video_id      = get_post_meta( $post_id, 'fjdf_testimonial_video_id', true ) ?: '30lFwLSgJHo';
	$video_thumb   = get_field( 'fjdf_testimonial_video_thumb', $post_id );

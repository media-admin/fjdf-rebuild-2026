...
function medialab_project_check_dependencies() {
    if (!function_exists('medialab_core_version')) {
        add_action('admin_notices', function() {
            ...
            echo '<strong>FJDF Rebuild 2026 Plugin</strong> requires ';
...
function medialab_project_init() {
    ...
    // Load text domain
    load_plugin_textdomain('fjdf', false, dirname(MEDIALAB_PROJECT_BASENAME) . '/languages');
...
function medialab_project_activate() {
    ...
    if (!medialab_project_check_dependencies()) {
        ...
        wp_die(
            '<strong>FJDF Rebuild 2026 Plugin</strong> requires <strong>Media Lab Agency Core</strong> plugin.',
